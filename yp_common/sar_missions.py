...
def build_mission_items(
    waypoints: list,
    *,
    force_guided_on_complete: bool = False,
    surface_vehicle: bool = False,
    parameter_overrides: bool = True,
) -> list[tuple]:
    """Convert UI mission entries to the tuples accepted by ``upload_mission``.

    Preserve bridge-specific surface altitude and parameter-override behavior.
    Entries without coordinates are skipped; explicit MAVLink command IDs and
    parameters take precedence over the UI's waypoint defaults.
    """
    item_type_to_cmd = {
    ...
    for wp in waypoints:
        ...
        defaults = (
            ...
            float(wp.get("acceptance_radius_m", 8.0)),
            0.0,
            float(wp.get("yaw_deg", 0.0) or 0.0),
        )
        params = tuple(
            float(wp.get(f"param{index}", default)) if parameter_overrides else default
            for index, default in enumerate(defaults, 1)
        )
        items.append((
            float(lat), float(lon),
            0.0 if surface_vehicle else float(wp.get("altitude", 30.0)),
            command_id, *params,
        ))
